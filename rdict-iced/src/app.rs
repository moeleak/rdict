...
use material::widget::text_input;
use material::widget::{button, progress_bar, select, theme_picker};
...
use crate::render;
use crate::{android_system_fonts, android_top_safe_area, sync_android_system_bars};
...
impl Default for State {
    fn default() -> Self {
        ...
        sync_android_system_bars_for_theme(&state.theme_controller);
...
fn sync_android_system_bars_for_theme(theme_controller: &theme_picker::ThemeController) {
    let theme = theme_controller.theme("Rdict");
    let colors = theme.colors();

    sync_android_system_bars(theme_controller.dark_mode(), colors.surface.color);
}
...
fn view(state: &State) -> material::Element<'_, Message> {
    ...
    let mut layout = column![
        row![
            search_input(&state.text_input_content),
            select::outlined(
                &LANGUAGES[..],
    ...
    .padding(Padding {
        top: 10.0 + android_top_safe_area(),
        right: 10.0,
        bottom: 10.0,
        left: 10.0,
    })
...
fn search_input(value: &str) -> material::Element<'_, Message> {
    text_input::outlined_floating("Type something here", value)
